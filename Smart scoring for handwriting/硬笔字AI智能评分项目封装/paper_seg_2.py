...
def mask1(img):
    ...
    cv2.circle(mask,(83,83),40,(1,1,1),-1)
    cv2.circle(mask1,(83,83),40,(0,0,0),-1)
    ...
    for m in range(1,2):
        for i in range(6):
            for j in range(4):
                ...
                img_bi = cv2.threshold(img1,120,255,type=0)[1]
                img_bi = np.where(img_bi>120,0,1)
                sum1=np.sum(img_bi)
                k=k+1
                if sum1>100: 
                    path=os.path.join(output_dir,"img{}.jpg".format(k))
...
def run(request_data):
    ...
    img1=result_img1[int(m/2):-200,int(n/2)-20:int(n/2)+20]
 
    gray=cv2.cvtColor(img1,cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray,(3,3),0)
    img_bi = cv2.threshold(gray,150,255,type=0)[1]
    edges = cv2.Canny(img_bi, 50, 150, apertureSize = 3)
    edges = np.hstack([edges,edges])
    edges = np.hstack([edges,edges])
    edges = np.hstack([edges,edges])
